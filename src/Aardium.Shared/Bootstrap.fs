...
module private Utilities =
    ...
    let failf fmt =
        Printf.kprintf (fun str ->
            failwithf "[Aardium.Shared] %s" str
        ) fmt
...
module private Strings =
    ...
    let binaryName =
        match platform with
        | Platform.Win    -> "Aardium.Shared.exe"
        | Platform.Linux  -> "Aardium.Shared"
        | Platform.Darwin -> "Aardium.Shared.app/Contents/MacOS/Aardium.Shared"
        | platform ->
    ...
    let defaultCachePath =
        Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Aardium.Shared")
...
type Aardium =
    ...
    static member Init([<Optional; DefaultParameterValue(null: string)>] path: string) =
        lock initLock (fun _ ->
            Report.BeginTimed("Initializing Aardium.Shared")
                ...
                if not Aardium.IsInitialized then
                    ...
                    try
                        ...
                        | Strings.Platform.Linux | Strings.Platform.Darwin when not Aardium.IsInitialized ->
                            ...
                            let tarPath = Path.Combine(toolsPath, $"Aardium.Shared-%s{Strings.platform}-%s{Strings.architecture}.tar.gz")
